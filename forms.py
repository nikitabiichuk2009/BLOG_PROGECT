...
def starts_with_capital_for_author(form, field):
    ...
    for word in words:
        first_char = word[0]
        if not (first_char.isupper() and not any(char.isdigit() or char in special_characters for char in word)):
            raise ValidationError(
                "Each word should start with a capital letter, can't contain numbers or special characters!")


class RegisterForm(FlaskForm):
    name = StringField("Full Name", validators=[DataRequired(), Length(min=10), starts_with_capital_for_author])
